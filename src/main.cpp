...
RGBAColor white = RGBAColor(255, 255, 255, 255);
auto red = RGBAColor(255, 0, 0, 255);
auto green = RGBAColor(0, 255, 0, 255);
RGBAColor colors[3] = {white, red, green};
int main2(int, char **) {
  auto width = 250;
  auto height = 250;
  ...
  // draw::triangle(Vec2i(120, 50), Vec2i(120, 80), Vec2i(150, 30), image,
  // green); draw::triangle(Vec2i(120, 50), Vec2i(120, 80), Vec2i(150, 30),
  // image, green);
...
void renderHeadFlatFast() {
  ...
  for (auto i = 0; i < model.nfaces(); i++) {
    ...
    auto cc = RGBAColor(rand() % 255, rand() % 255, rand() % 255, 255);
...
void renderHeadShaded() {
  ...
  for (auto i = 0; i < model.nfaces(); i++) {
    ...
    auto zz = (model.vert(face.vertIdx_[2]) - model.vert(face.vertIdx_[0])) ^
    ...
    // printf("%f\n", ivec * 255);
    if (ivec > 0) {
      auto cc = RGBAColor(ivec * 255, ivec * 255, ivec * 255, 255);
...
void renderHeadWireframe(int, char **) {
  ...
  // For each face in the mesh we want to draw the lines which make up the
  ...
  for (auto i = 0; i < model.nfaces(); i++) {
    ...
    for (auto j = 0; j < 3; j++) {
      ...
      auto v1 = model.vert(
          face.vertIdx_[(j + 1) %
                        3]); // We take mod 3 to always pick a vertex we haven't
                             // connected to yet so v0 @j=0 ==> v1 @(j+1)%3=1;
                             // v0 @j=2 ==> v1 @(j+2)%3 = 0

...
public:
  ...
  // Assigment operator, we need to decrement the ref count on the old object,
  // and increment on the new object
  Reference &operator=(const Reference<T> &r) {
    // Increment the refcount on the incomming object
    if (r.m_ptr)
      (r.m_ptr->nReferences)++;

    // Decrement the old refcount -- if we hit zero, free the memory as no one
    // else is using it...
  ...
  ~Reference() {
    // Decrement refcount and check for 0 refs -- delete if we are the last
    // reference
    if (m_ptr && --(m_ptr->nReferences) == 0)
      delete m_ptr;
  }
  ...
  bool operator<(const Reference<T> &r2) const { return m_ptr < r2.m_ptr; }
...
public:
  ...
  T *Alloc() {
    if (nAvailable == 0) {
      int nAlloc =
          std::max((unsigned long)16, (unsigned long)(65536 / sizeof(T)));
  ...
  void *Alloc(uint64_t sz) {
    ...
    if (m_curBlockPos + sz > m_blockSize) {
      ...
      if (m_availableBlocks.size() && sz <= m_blockSize) {
        ...
        m_currentBlock = (char *)AllocAligned(std::max(sz, m_blockSize));
      }
      m_curBlockPos = 0;
    }

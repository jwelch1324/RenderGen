...
public:
  ...
  bool operator==(const Normal<T> &o) const {
  ...
  Normal<T> operator+(const Normal<T> &n) {
    return Normal<T>((T)(x + n.x), (T)(y + n.y), (T)(z + n.z));
  }
  Normal<T> &operator+=(const Normal<T> &n) {
    x += n.x;
    y += n.y;
  ...
  Normal<T> operator*(const Float f) const {
    return Normal<T>((T)(x * f), (T)(y * f), (T)(z * f));
  }
  ...
  inline Float norm() const { return sqrtf(LengthSquared()); }
